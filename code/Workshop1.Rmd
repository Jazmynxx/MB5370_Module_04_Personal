...
#hwy= fuel efficiency in miles per gallon
?mpg
```
...
# Question- what happens if u map an aesthetic to something other than a variable name?
```{r}
ggplot(data = mpg) + 
...
?facet_wrap
```
nrow and ncol dont do anything in facet wrap
...
ggplot(data = mpg) + 
  ...
  geom_smooth(mapping = aes(x = displ, y = hwy, colour = "pink")) # try smooth line
```
...
```{r}
ggplot(data = mpg) + 
  geom_smooth(mapping = aes(x = displ, y = hwy, linetype = drv, colour = drv)) # Can also use "lty = " for the same result
```
...
#Line
ggplot (data = mpg) +
  geom_line(mapping = aes(x=displ, y = hwy, colour = "coral"), 
             show.legend = FALSE)
#boxplot
ggplot (data = mpg) +
...
moving on to a new data set, diamonds
